...
class SettingsScreen(BaseScreen):
    ...
    def build(self) -> None:
        ...
        ys = [self.card.y + 60 + index * 61 for index in range(6)]
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        x, y = self.card.x + 42, self.card.y + 60
        ...
        for label, description in rows:
            text(surface, label, (x, y), 14, WHITE, bold=True); text(surface, description, (x, y + 21), 10, MUTED); y += 61
        for button in (self.speed_button, self.sound_button, self.resolution_button, self.currency_button, self.autosave_button, self.save_button): button.draw(surface)
